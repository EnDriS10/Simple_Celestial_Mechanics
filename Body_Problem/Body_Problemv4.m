...
function [x,y,z] =Simul(M,r, v, p, Tf, dt)
 ...
 for step=1:Tf
    ...
    F_inicial = ComputeF(M, r);  
    p_half = p + 0.5.*F_inicial.*dt;
    r = r + (p_half.*dt)./(M.*M_rr);
    F_new = ComputeF(M,r);
    p = p_half + 0.5.*F_new.*dt;
...
function simularYAnimar(M, r, v, p, D, Tf, dt)
    ...
    for step = 1:Tf
        % Calcular fuerzas gravitacionales
            ...
            F_inicial = ComputeF(M, r_current);  
            p_half = p_current + 0.5.*F_inicial.*dt;
            r_current = r_current + (p_half.*dt)./(M.*M_rr);
            F_new = ComputeF(M,r_current);
            p_current = p_half + 0.5.*F_new.*dt;
